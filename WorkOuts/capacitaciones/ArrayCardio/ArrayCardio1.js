...
const sortedNameInventors = inventors.sort((a,b)=>{
  if(a.first_name < b.first_name ){
      return -1;
  }
});

const mappedInventorsName=inventors.map(inventor=>{
  return inventor.first_name+" "+inventor.last_name;
})



/* const reducedInventorsAge=inventors.reduce((total,inventor)=>{
  return total+inventor.age;
},0)  */
const añoActual=new Date().getFullYear();
const reducedInventorsDate=inventors.map(inventor=>{
  return{
    fullName: inventor.first_name+" "+inventor.last_name,
age:añoActual-inventor.birth_date
  }
...
console.table(sortedNameInventors);
console.table(mappedInventorsName);
console.table(reducedInventorsDate);